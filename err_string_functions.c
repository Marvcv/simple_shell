...
int _eputchar(char c)
{
	...
	/* Check if the buffer is full or if a flush is requested */
	...
	/* Add the character to the buffer if not flushing */
	if (c != BUF_FLUSH)
		buf[i++] = c;
...
int _putfd(char c, int fd)
{
	...
	if (c == BUF_FLUSH || i >= WRITE_BUF_SIZE)
	{
		write(fd, buf, i); /* Write the buffer to the specified file descriptor */
		i = 0; /* Reset the buffer index */
	}
